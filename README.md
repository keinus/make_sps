...
### 1. 폴더 구조 생성
각 장비별로 별도의 폴더를 생성합니다. 각 폴더 내에는 CSU별로 소스코드 파일이 저장됩니다. 폴더 구조는 다음과 같습니다:
...
### 2. project.yaml 파일 작성
각 장비별 폴더에 project.yaml 파일을 하나씩 작성합니다. 이 파일은 해당 장비의 프로젝트 정보를 포함하고 있습니다.
...
각 장비별 폴더를 ZIP 형식으로 압축합니다.

1. device-1 폴더를 선택합니다.
2. 압축 프로그램을 사용하여 해당 폴더를 ZIP 파일로 압축합니다.

### 4. UI에서 파일 업로드

압축한 ZIP 파일을 UI에서 업로드합니다.

### 추가 정보

UI에서 입력하는 각 정보(장비식별자, CSU 등)는 디버그 및 예비용으로 사용됩니다.
