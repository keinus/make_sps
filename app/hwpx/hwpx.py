...
class HWPXMLBuilder:
    ...
    def add_para_number_text(self, text: str, level: int = 1, style: str = "1") -> None:
        ...
        # 텍스트 요소 생성
        text_elem = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}t')
        text_elem.text = text
    ...
    def _create_table_element(self, 
        ...
        # 테이블 크기 설정
        sz_elem = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}sz')
        ...
        # 위치 설정
        pos_elem = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}pos')
        ...
        # 외부 여백 설정
        out_margin = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}outMargin')
        ...
        # 캡션 설정
        caption_elem = self._create_caption_element(caption, table_num, caption_suffix)
        tbl.append(caption_elem)
        
        # 내부 여백 설정
        in_margin = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}inMargin')
    ...
    def _create_caption_element(self, caption, table_num, caption_suffix):
        """캡션 요소 생성"""
        caption_elem = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}caption')
        ...
        # 서브리스트 생성
        sublist = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}subList')
        sublist.set('id', '')
        ...
        autonum = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}autoNum')
        autonum.set('num', str(table_num))
        autonum.set('numType', 'TABLE')
        
        autonum_format = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}autoNumFormat')
    ...
    def _create_header_row(self, headers: list[str], sizes: list[int]):
        ...
        for i, header in enumerate(headers):
            ...
            # 셀 주소
            cell_addr = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}cellAddr')
            cell_addr.set('colAddr', str(i))
            cell_addr.set('rowAddr', '0')
            tc.append(cell_addr)
            
            # 셀 스팬
            cell_span = ET.Element('{http://www.hancom.co.kr/hwpml/2011/paragraph}cellSpan')
    ...
    def _add_table_data_rows(self, tbl_elem, table_data: List[List[str]], sizes: list[int], col_count: int):
        ...
        for row_data in table_data:
            if row_data[0].startswith("저장위치:"):
                storage_row = self._create_storage_location_row(row_data[0], col_count, row_index)
