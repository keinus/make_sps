...
def count_code_lines(filepath) -> int:
    ...
    for line in lines:
        ...
        if in_multiline_comment:
            ...
            if end_pos != -1:
                # 주석 종료 후 남은 부분 처리
                remaining = stripped_line[end_pos +
                                          len(current_ml_end_pattern):].strip()
                in_multiline_comment = False
                current_ml_end_pattern = None
        ...
        # 한줄 주석으로 시작하는지 먼저 확인
        is_single_comment_only = _is_comment_line(
            stripped_line, single_line_comment_patterns)
        ...
        for start_pattern, end_pattern in ml_comment_patterns.items():
            ...
            if start_pos != -1:
                ...
                end_pos = stripped_line.find(
                    end_pattern, start_pos + len(start_pattern))
                if end_pos != -1:
                    # 주석이 같은 줄에서 끝남
                    code_after = stripped_line[end_pos +
                                               len(end_pattern):].strip()
